...
const DaGalleryView = (props: IDaGalleryViewProps) => {
    const [galleryData, setGalleryData] = useState<IGalleryGet[]>([])
    const [imgSize, setImgSize] = useState<number>(200)
    let params = useParams();

    const loadGalleries = async () => {
        const response = await fetch(`${props.apiUrl}/api/da/gallery/${params.artistid}`);
    ...
    const generateGalleryCard = (gallData: IGalleryGet) => {
        return (
            <a href={`${props.apiUrl}/grid/da/gallery/${gallData.ID}`}>
                <Card
            ...
            <Row>
                <Col span={24}>
                    <Space size={[8, 16]} wrap>
                        {galleryData.map((gall) => generateGalleryCard(gall))}
